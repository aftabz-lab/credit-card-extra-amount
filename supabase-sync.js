...
function normalizedSession(raw) {
  ...
  return {
    ...
    expires_at: raw.expires_at ? Number(raw.expires_at) * (Number(raw.expires_at) < 1e12 ? 1000 : 1) : Date.now() + expiresIn * 1000,
    user: raw.user || null,
  };
}
...
// Credit Card uses an atomic timestamp comparison so two publishers cannot
// silently replace each other's mappings. Existing dashboards keep their API.
export async function publishCloudSnapshotIfUnchanged(snapshotKey, payload, expectedUpdatedAt) {
  async function send(session) {
    ...
    if (expectedUpdatedAt) {
      ...
      query.searchParams.set('updated_at', `eq.${expectedUpdatedAt}`);
    }
    const response = await fetch(query, {
      method: expectedUpdatedAt ? 'PATCH' : 'POST',
      headers: { ...restHeaders(session.access_token), Prefer: 'return=representation' },
    ...
    if (response.status === 409) throw new Error('Another publisher created this snapshot. Refresh before publishing.');
    if (!response.ok) {
      const message = await response.text();
      throw new Error(`Snapshot publish failed (${response.status}). ${message.slice(0,300)}`);
    }
    const changed = await response.json();
    if (!Array.isArray(changed) || changed.length !== 1) throw new Error('The shared snapshot changed during publication. Your local draft was kept.');
    return { row: changed[0] };
  }
  let session = await validPublisherSession(false);
  if (!session) throw new Error('Publisher login required on this source PC.');
  let result = await send(session);
  if (result.expired) {
    session = await validPublisherSession(true);
    if (!session) throw new Error('Publisher session expired. Sign in again.');
    result = await send(session);
    if (result.expired) throw new Error('Publisher authorization failed. Sign in again.');
  }
  recordSync(snapshotKey, { ok: true, reason: 'published' });
  return result.row;
}
