...
function worksheetXml(model,hasChart){
  const headers=['RHO / Leader','Zonal','Outlets',...model.banks.map(b=>b.label),'Total Extra Amount','Month End Projection','Share'];
  ...
  model.rows.forEach((row,index)=>{
    const n=firstDataRow+index;const subtotal=row.kind==='leader-total';const textStyle=subtotal?8:4;const countStyle=subtotal?9:5;const moneyStyle=subtotal?10:6;const shareStyle=subtotal?11:7;
    const values=[textCell(`A${n}`,row.leader,textStyle),textCell(`B${n}`,row.zonal,textStyle),numberCell(`C${n}`,row.outlets,countStyle)];
    row.channels.forEach((value,channelIndex)=>values.push(numberCell(`${columnName(3+channelIndex)}${n}`,value,moneyStyle)));
    const extraColumn=3+model.banks.length;values.push(numberCell(`${columnName(extraColumn)}${n}`,row.extra,moneyStyle),numberCell(`${columnName(extraColumn+1)}${n}`,row.projection,moneyStyle),numberCell(`${columnName(extraColumn+2)}${n}`,row.share,shareStyle));
    sheetRows.push(rowXml(n,values,19));
  });
  const totalCells=[textCell(`A${totalRow}`,'VIEW TOTAL',8),textCell(`B${totalRow}`,'ALL ZONALS',8),numberCell(`C${totalRow}`,model.totals.outlets,9)];
  model.totals.channels.forEach((value,index)=>totalCells.push(numberCell(`${columnName(3+index)}${totalRow}`,value,10)));
  const extraColumn=3+model.banks.length;totalCells.push(numberCell(`${columnName(extraColumn)}${totalRow}`,model.totals.extra,10),numberCell(`${columnName(extraColumn+1)}${totalRow}`,model.totals.projection,10),numberCell(`${columnName(extraColumn+2)}${totalRow}`,model.totals.share,11));sheetRows.push(rowXml(totalRow,totalCells,21));
...
export function createManagementWorkbook(model){
  ...
  const normalized={...model,banks:Array.isArray(model.banks)?model.banks:[],rows:Array.isArray(model.rows)?model.rows:[],chartRows:Array.isArray(model.chartRows)?model.chartRows:[],totals:model.totals||{outlets:0,channels:[],extra:0,projection:null,share:0}};
  ...
  if(hasChart)files.push(['xl/worksheets/_rels/sheet1.xml.rels',sheetRels],['xl/drawings/drawing1.xml',drawingXml(6+normalized.banks.length)],['xl/drawings/_rels/drawing1.xml.rels',drawingRels],['xl/charts/chart1.xml',chartXml(normalized)]);
